...
// ── Week 2 Day 7 — closing a real integration gap ────────────────────────
// Day 4's 'secure-api' middleware group (ApiKeyMiddleware +
// ResponseTimeLoggerMiddleware) has, until today, only ever protected a
// standalone demo route (/api/secure/ping) — it never actually guarded
// the real Task API. That's a genuine gap: a reasonable real-world design
// is "reads are public, writes require the key," and this controller now
// implements exactly that.
//
// HasMiddleware is a Laravel 11 feature: implementing it and returning
// Middleware definitions from a static middleware() method lets a
// controller declare its OWN middleware, scoped to specific actions —
// without touching routes/api.php at all. index() and show() stay
// completely public; store(), update(), destroy(), and markAsDone() now
// require a valid X-Api-Key header.
//
// Honest caveat: I'm confident in this feature's existence and general
// shape from documented Laravel 11 behavior, but I don't have vendor/ in
// this sandbox to verify Middleware's exact constructor signature against
// source. Worth checking
// vendor/laravel/framework/src/Illuminate/Routing/Controllers/Middleware.php
// once you run composer install if anything here doesn't match.
